...
- **Macro- and industry-level analysis; where is SSS vs history; what is most useful for predicting SSS?** On **Macro & industry** I tie **spend-equiv** (**{macro_data.SPEND_EQUIV_BASE_YEAR}** basket), peer **SSS / pricing / traffic**, and **CPI–PPI–commodities** into one narrative: the peer group ran **hot** post-Covid as pricing offset input inflation, then **2025** shows **traffic softening** after years of mid-single-digit increases—more **give-back in traffic** than a collapse in the macro backdrop. For predicting SSS without any paid data, the highest-value layer is still pulling metrics from filings: **traffic**, **ticket**, and **mix**. **OpenTable** seated diners add an industry demand check; **NYC/SF** transit is a narrow office-commuter read; **Google Trends** / **GDELT** add **awareness**. Broad **national macro** data is best treated as broad context, not a chain-specific model engine. None are nearly as helpful as **transaction data**.

- **SSS forecasts (CMG, CAVA, BROS): which data has better predictive power for SSS; four quarters forward.** I chose to make forecasts for **CMG**, **CAVA**, and **BROS** because they are the largest in the basket. **Company-reported drivers** have the best “predictive” link to **SSS** here because they *are* the same decomposition management uses. **Macro** is largely common across the three. Among free proxies, **GTrends** and **GDELT** are useful for company-level consumer attention; factored in slightly to traffic. **BROS** needs extra work on **franchise vs company** reporting that I didn't model. Forecasts for the drivers were determined using management commentary and other contextual data. 
I used AUV, 2-yr stack comp, and digital mix as a sanity check on the forecasts.
